...
class AthleteSEO {
    ...
    generateTitle(athlete) {
        if (!athlete) return '🥋 Спортсмен збірної України з дзюдо | Профіль атлета ФДУ';
        
        const emoji = this.genderEmoji[athlete.gender] || this.genderEmoji.default;
        const weight = athlete.weight ? ` (${athlete.weight} кг)` : '';
        
        return `${emoji} ${athlete.name} - дзюдоїст збірної України${weight} | ФДУ`;
    }
    ...
    generateDescription(athlete) {
        if (!athlete) return 'Профіль спортсмена збірної України з дзюдо: біографія, досягнення, статистика змагань атлета ФДУ';
        
        let description = `Профіль ${athlete.name} - спортсмена збірної України з дзюдо. `;
        ...
        description += 'Біографія та досягнення на сайті ФДУ.';
